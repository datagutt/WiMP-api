...
function WiMP(config){
	if(!(this instanceof WiMP)) return new WiMP(_.merge(Config, config));
...
WiMP.prototype.getFeatured = function(fn){
	var self = this;
	var parsedItems = [];
	self.request('GET', 'promotions', null, null, function(err, res){
		var items = res.body.items;
		if(items){	
			_.each(items, function(item){
				if(item.type == 'PLAYLIST'){	
					self._parseFeaturedPlaylist(item, function(err, i){
						parsedItems.push(i);
					});
				}
			});
...
WiMP.prototype._parseAlbum = function(item, fn){
	...
	var album = new Album({
		'id': item.id,
		'name': item.title,
		'numTracks': parseInt(item.numberOfTracks, 10),
...
WiMP.prototype._parseTrack = function(item, fn){
	...
	var track = new Track({
		...
		'name': item.title,
		'duration': parseInt(item.duration, 10),
		'trackNum': parseInt(item.trackNumber, 10),
		'discNum': parseInt(item.volumeNumber, 10),
		'popularity': item.popularity,
		'artist': item.artist,
		'album': item.album,
		'available': item.streamReady
	}, self);
	if(fn){
		fn(null, track);
	}
...
WiMP.prototype._parseFeaturedPlaylist = function(item, fn){
	var self = this;
